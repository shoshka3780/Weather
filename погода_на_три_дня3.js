...
async function fetchWeather() {
    ...
    async function findWorld(name) {
        const response = await fetch(`https://api.weatherapi.com/v1/search.json?key=b76243c0b1d64157974120632241107&q=${name}&lang=ru`)
        const res = await response.json()
        return res
    }    
    current.addEventListener("click", function() {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            fetchWeather(localStorage.getItem("lat"),localStorage.getItem("lon")).then(data=> {
                ...
                localStorage.setItem("name", data.location.name)
        ...
        city.addEventListener("input", function() {
         ...
         findWorld(cityName).then(data => {
            ...
            for (const element of data) {
                if(element.country !== "Россия") {
                list.insertAdjacentHTML("beforeend", `<li class = "other">${element.name} (${element.region})${element.country}</li>`)
                arrCityNotRussia.push({
                    name: `${element.name} ${element.region} ${element.country}`,
                    lat: element.lat,
                    lon: element.lon
                })  
            }
