window.onload = function() {
    ...
    let listArr = document.querySelectorAll(".hint-ul > li")
...
async function fetchWeather() {
    ...
    async function find(lat, lon) {
        const response = await fetch(`https://geocode-maps.yandex.ru/1.x/?apikey=a7fd4fdd-961c-44a4-9c17-17eb79dc107f&geocode=${lat},${lon}&lang=ru_RU&format=json&kind=locality&results=10`)
        const forecast = await response.json()
    return forecast
    }    
    todayPage.addEventListener("click", function() {
    ...
    async function addimg(dayTime, day, sunRise, sunSet, data) {
        ...
        if(localStorage.getItem("theme") === "dark"){
            ...
            if(code === 1066 || code === 1114 || code === 1117 || code === 1204 || code === 1207 || code === 1210 || code === 1216 || code === 1219 || code === 1222 || code === 1225) {
   ...
   function addDate() {
    ...
    const data = await fetchWeather().then(data => data)
        let sunRise = await addSun("sunrise", day, data)
        let sunSet = await addSun("sunset", day, data)    
        await addimg(dayTime,day,sunRise,sunSet, data)
        await addTemp(dayTime, day, sunRise, sunSet, data) 
        await addWind(dayTime, day, sunRise, sunSet, data)
        await addPress(dayTime, day, sunRise, sunSet, data)
        await addHum(dayTime, day, sunRise, sunSet, data)
        await addCityName(data)
...
function hideForm() {
    ...
    for (let j = 0; j < 10; j++) {
        listArr[j].style.display = "none"
        listArr[j].textContent =  ""
    }
}
...
city.addEventListener("input", function() {
    ...
    find(city.value).then(data => {
       if(city.value !== "") {
        let geoObject = data.response.GeoObjectCollection.featureMember
          for (let i = 0; i < geoObject.length; i++) {
            if(geoObject[i].GeoObject.metaDataProperty.GeocoderMetaData.kind === "locality") {
            let region = geoObject[i].GeoObject.description.split(",")
            let nameLocality = geoObject[i].GeoObject.name + " " + region.at(-1)
            let cords = geoObject[i].GeoObject.Point.pos.split(" ")
            listArr[i].style.display = "block"
            listArr[i].textContent =  geoObject[i].GeoObject.name + " " + geoObject[i].GeoObject.description
           cordsArr.push({
            lat: cords[1],
            lon: cords[0],
            name:  nameLocality,
...
listArr.forEach((item, index) => item.addEventListener("click", function() {
    for (let i = 0; i < cordsArr.length; i++) {
        if(index === cordsArr[i].index) {
            localStorage.removeItem("lon")        
            localStorage.removeItem("lat")
            localStorage.setItem("lon" , cordsArr[i].lon)        
            localStorage.setItem("lat", cordsArr[i].lat)
        localStorage.removeItem("name")
        localStorage.setItem("name", `${cordsArr[i].name}`) 
...
current.addEventListener("click", function() {
    navigator.geolocation.getCurrentPosition(position => {
        ...
        find(localStorage.getItem("lon"), localStorage.getItem("lat")).then(data => {
            let region = data.response.GeoObjectCollection.featureMember[0].GeoObject.description.split(",")
            localStorage.removeItem("name")
            localStorage.setItem("name", `${data.response.GeoObjectCollection.featureMember[0].GeoObject.name + region.at(-1)}`)
            location.reload()
        })
    })
